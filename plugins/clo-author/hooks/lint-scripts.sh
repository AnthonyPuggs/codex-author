#!/bin/bash
# lint-scripts.sh -- Mechanical grep-based linter for R, Python, and Julia scripts.
# Advisory only: it reports findings and exits 0.
#
# Usage:
#   plugins/clo-author/hooks/lint-scripts.sh <file>
#   plugins/clo-author/hooks/lint-scripts.sh <dir>
...
lint_file() {
  ...
  if [[ "$ext" == "R" ]]; then
    ...
    while IFS=: read -r num _; do
      [[ -n "$num" ]] && add_finding "LOW" "$num" "print() -- prefer message() for status output"
    ...
    done < <(grep -nE '1:(length|nrow|ncol|NROW|NCOL)\(' "$file" 2>/dev/null | grep -v '^[[:space:]]*#' || true)
    ...
    if [[ -n "$seed_line" && "$seed_line" -gt 30 ]]; then
      add_finding "MEDIUM" "$seed_line" "set.seed() should be near the top of the script"
    fi
  fi
  ...
  if [[ "$ext" == "py" ]]; then
    ...
    while IFS=: read -r num _; do
      [[ -n "$num" ]] && add_finding "MEDIUM" "$num" "np.random.seed() global state -- prefer default_rng(seed)"
    done < <(grep -n 'np\.random\.seed(' "$file" 2>/dev/null | grep -v '^[[:space:]]*#' || true)
    ...
    while IFS=: read -r num _; do
      if [[ -n "$num" && "$num" -gt 30 ]]; then
        add_finding "LOW" "$num" "Import after line 30 -- move imports near the top"
      fi
    done < <(grep -nE '^[[:space:]]*(import |from [^ ]+ import )' "$file" 2>/dev/null || true)

    while IFS=: read -r num _; do
      [[ -n "$num" ]] && add_finding "HIGH" "$num" "pip install in script -- use requirements or a lockfile"
    done < <(grep -n 'pip install\|subprocess.*pip' "$file" 2>/dev/null | grep -v '^[[:space:]]*#' || true)
    ...
    if grep -q 'np\.random\.\|random\.\|torch\.rand' "$file" 2>/dev/null; then
      if ! grep -q 'default_rng\|random\.seed\|manual_seed\|SEED' "$file" 2>/dev/null; then
        add_finding_noline "HIGH" "Stochastic code detected but no seed policy was found"
      fi
    fi
  fi
  ...
  if [[ "$ext" == "jl" ]]; then
    ...
    while IFS=: read -r num _; do
      [[ -n "$num" ]] && add_finding "MEDIUM" "$num" "eval/@eval at runtime -- prefer multiple dispatch"
    done < <(grep -n '@eval\|eval(' "$file" 2>/dev/null | grep -v '^[[:space:]]*#' || true)

    if grep -q 'rand(\|randn(\|shuffle' "$file" 2>/dev/null; then
      if ! grep -q 'MersenneTwister\|Random\.seed!\|SEED' "$file" 2>/dev/null; then
        add_finding_noline "HIGH" "Stochastic code detected but no RNG seed policy was found"
      fi
    fi

    while IFS=: read -r num _; do
      if [[ -n "$num" && "$num" -gt 30 ]]; then
        add_finding "LOW" "$num" "using/import after line 30 -- move imports near the top"
      fi

...
- Added `AGENTS.md` as the active root constitution for Codex
- Added a repo-local Codex plugin at `plugins/clo-author/`
- Added repo marketplace registration at `.agents/plugins/marketplace.json`
- Added 10 Codex-native skills for the full research pipeline
- Added active Codex references for workflow, delegation, hook replacement, and skill mapping

### Migration of Research Infrastructure
- Migrated domain and journal calibration into `plugins/clo-author/references/`
- Preserved Claude-era agents, rules, and hooks under explicit `legacy-*` locations for source continuity
- Replaced the root `CLAUDE.md` with a deprecation note pointing to `AGENTS.md`

### Documentation Rewrite
- Rewrote README for Codex-native onboarding
- Rewrote the Quarto guide around skills, `AGENTS.md`, and repo-local plugin structure
- Updated the skill template to target `plugins/clo-author/skills/`
- Updated governance and exploration docs to point at the Codex workflow

### Persistence Model
- Standardized file-backed persistence around `MEMORY.md`, `quality_reports/specs/`, `quality_reports/plans/`, and `quality_reports/session_logs/`
- Replaced hidden memory and hook assumptions with explicit recovery guidance
